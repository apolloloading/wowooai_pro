 ...
 * `window.WowooAI` so plugin bundles can register routes and tool renderers
...
/** Shared host dependencies exposed to plugin bundles via `window.WowooAI.host`. */
...
export function installHostExternals(): void {
  ...
  if (!window.WowooAI) {
    (window as any).WowooAI = {} as WindowNamespace;
  }

  if (!window.WowooAI.host) {
    window.WowooAI.host = {
      React,
      ReactDOM,
      antd,
  ...
  if (!window.WowooAI.registerRoutes) {
    window.WowooAI.registerRoutes = (pluginId, routes) => {
  ...
  if (!window.WowooAI.registerToolRender) {
    window.WowooAI.registerToolRender = (pluginId, renderers) => {

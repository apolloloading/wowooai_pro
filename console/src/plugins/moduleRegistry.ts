 ...
 * 2. Plugins access and modify module exports via window.WowooAI.modules
...
class ModuleRegistryImpl implements ModuleRegistry {
  ...
  /**
   * Get all modules (for window.WowooAI.modules)
   */
...
// Expose to window.WowooAI.modules (for plugin use)
// Set during initialization
if (typeof window !== "undefined") {
  if (!window.WowooAI) {
    (window as any).WowooAI = {};
  }

  // Use Proxy for dynamic access, ensuring plugins always get latest module state
  Object.defineProperty(window.WowooAI, "modules", {
    get() {

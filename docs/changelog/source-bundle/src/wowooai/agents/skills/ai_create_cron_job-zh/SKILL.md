...
- **Renliwo 关键字检测（强制）**：当用户原话命中以下任一关键字时，**必须**在 `request.input` 文本最前面追加前缀 `[使用工具：renliwo_browser] `（带尾随空格），用户原话整体保留：
  - 域名：`renliwo.com`、`ereference-v-uat.renliwo.com`
  - 业务关键词：`Renliwo`、`renliwo`、`人力沃`、`合同产品列表`、`综合管理`、`合同管理`、`认款`、`渠道费`、`费用结算`
  - 例：用户说"每天 0 点登录 renliwo 导出合同产品列表" → `request.input` 文本写成 `[使用工具：renliwo_browser] 每天 0 点登录 renliwo 导出合同产品列表`
  - 原因：`browser_use` 对 *.renliwo.com 已加硬拦截，必须强制路由到 `renliwo_browser`，否则任务执行时会被拒绝。

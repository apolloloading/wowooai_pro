...
In these cases, use the standard `start` (which defaults to headed/visible window), adding `headed=false` only if the user explicitly requests headless — refer to **browser_visible**.
...
- This **attaches to an external browser**, not a new process launched by wowooai
...
To launch in headless mode (only when user explicitly requests it):

```json
{"action": "start", "headed": false, "cdp_port": 9222}
```
...
### 1. Managed CDP launched by wowooai
...
These browsers are launched and owned by wowooai. `stop` will:
...
These browsers are not launched by wowooai. `stop` will only:
...
- The default launch mode is **headed (visible window)**; only pass `headed=false` when the user explicitly requests headless/background mode

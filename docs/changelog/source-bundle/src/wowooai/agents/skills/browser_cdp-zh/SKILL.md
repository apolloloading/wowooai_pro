...
这些情况通常直接使用普通 `start`（默认就是有头/可见窗口），仅当用户明确要求"无头"时才加 `headed=false`，参考 **browser_visible** 即可。
...
- 这是**附着到外部浏览器**，不是 wowooai 自己启动的新进程
...
这类浏览器由 wowooai 自行启动并持有进程，`stop` 时会：
...
- 默认启动就是**有头模式（可见窗口）**，仅当用户明确要求"无头/后台"时才传 `headed=false`
- **登录/账号密码/验证码必须由用户在可见窗口里手动完成，禁止自动填充**

...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.wowooai or WORKING_DIR)
...
def _apply_legacy_qa_disable_for_migration(config) -> None:
    ...
    if getattr(ref, "enabled", True):
        ...
        logger.info(
            ...
            "(new wowooai builtin QA slot was created)",
            legacy_id,
        )
...
def ensure_qa_agent_exists() -> None:
    ...
    ``wowooai_source_index``), and built-in tools are restricted (see

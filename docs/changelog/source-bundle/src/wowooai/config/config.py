...
class MatrixConfig(BaseChannelConfig):
    ...
    username: str = ""
    password: str = ""
    device_name: str = "wowooai-worker"
...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is WowooAI. How can I help you?"
...
class SIPChannelConfig(BaseChannelConfig):
    ...
    language: str = "zh-CN"
    welcome_greeting: str = "你好，我是WowooAI"
...
class WeixinConfig(BaseChannelConfig):
    ...
    bot_token_file: Path to persist/load the bot_token
                    (default ~/.wowooai/weixin_bot_token).
...
class ScenePrompt(BaseModel):
    """Quick-start prompt shown on the chat welcome page.

    Each prompt is a clickable card.  Clicking it auto-sends ``value``
    to the agent on behalf of the user.  ``label`` is the short text
    displayed inside the card (emoji allowed).
    """

    label: str = Field(..., description="Display text on the card")
    value: str = Field(..., description="Text auto-sent on click")
...
class AgentProfileConfig(BaseModel):
    ...
    scene_prompts: List["ScenePrompt"] = Field(
        ...
        description=(
            "Quick-start prompts shown on the chat welcome page. "
            "Each item has a `label` (display text) and `value` "
            "(text auto-sent on click). "
            "Recommended length: 0~6 items."
        ),
    )
...
def _default_builtin_tools() -> Dict[str, BuiltinToolConfig]:
    ...
    return {
        ...
        "browser_use": BuiltinToolConfig(
            ...
            enabled=True,
            description=(
                "General browser automation for non-Renliwo websites. "
                "MUST NOT be used for *.renliwo.com URLs — those go through "
                "`renliwo_browser`."
            ),
            icon="🌐",
        ),
        "renliwo_browser": BuiltinToolConfig(
            ...
            enabled=True,
            description=(
                "Renliwo HR platform browser automation. MUST be used for "
                "ALL *.renliwo.com URLs (login, nav_menu, ant_select, "
                "export, etc.). Do NOT fall back to `browser_use` for "
                "Renliwo."
            ),
            icon="🧑‍💼",
        ),
...
def build_fallback_agent_profile_config(
    ...
    Used by :func:`load_agent_config` and ``wowooai doctor fix``
...
def load_agent_config(agent_id: str) -> AgentProfileConfig:
    ...
    # Normalize legacy ~/.wowooai-bound paths to current WORKING_DIR.
    # This keeps WOWOOAI_WORKING_DIR effective even if existing agent.json
    # contains older hard-coded paths like "~/.wowooai/media".
    try:
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default WowooAI agent",
    ...
    # When WOWOOAI_WORKING_DIR is customized, historical data may still exist
    # under "~/.wowooai".
    old_workspace = Path("~/.wowooai").expanduser().resolve()

...
- Builds the console frontend (`console/`), copies `console/dist` to `src/wowooai/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `wowooai:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/wowooai:v1 --no-cache`.

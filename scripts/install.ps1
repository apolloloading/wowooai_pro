# wowooai Installer for Windows (self-contained: includes uv download via GitHub)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
#                            [-Extras "dev,whisper"] [-UvPath PATH]
#
# Installs wowooai into ~/.wowooai with a uv-managed Python environment.
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$wowooaiHome     = if ($env:wowooai_HOME) { $env:wowooai_HOME } else { Join-Path $HOME ".wowooai" }
$wowooaiVenv     = Join-Path $wowooaiHome "venv"
$wowooaiBin      = Join-Path $wowooaiHome "bin"
$PythonVersion = "3.12"
$wowooaiRepo     = "https://github.com/agentscope-ai/wowooai.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[wowooai] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[wowooai] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[wowooai] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
wowooai Installer for Windows
...
Environment:
  wowooai_HOME            Installation directory (default: ~/.wowooai)
"@
    exit 0
}

Write-Host "[wowooai] " -ForegroundColor Green -NoNewline
Write-Host "Installing wowooai into " -NoNewline
Write-Host "$wowooaiHome" -ForegroundColor White
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\wowooai\console"
...
$Venvwowooai = Join-Path $wowooaiVenv "Scripts\wowooai.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing wowooai from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\wowooai"
        }
        Write-Info "Installing wowooai from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "wowooai-install-$(Get-Random)"
        try {
            git clone --depth 1 $wowooaiRepo $cloneDir
    ...
    $package = "wowooai"
    if ($Version) { $package = "wowooai==$Version" }

    Write-Info "Installing ${package}${ExtrasSuffix} from PyPI..."
    uv pip install "${package}${ExtrasSuffix}" --python $VenvPython --prerelease=allow --quiet --refresh-package wowooai
    if ($LASTEXITCODE -ne 0) { Stop-WithError "Installation failed" }
}

# Verify the CLI entry point exists
if (-not (Test-Path $Venvwowooai)) { Stop-WithError "Installation failed: wowooai CLI not found in venv" }

Write-Info "wowooai installed successfully"

# Check console availability (for PyPI installs, check the installed package)
if (-not $script:ConsoleAvailable) {
    $consoleCheck = & $VenvPython -c "import importlib.resources, wowooai; p=importlib.resources.files('wowooai')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
    if ($consoleCheck -eq "yes") { $script:ConsoleAvailable = $true }
}

# ── Step 4: Create wrapper scripts ───────────────────────────────────────────
New-Item -ItemType Directory -Path $wowooaiBin -Force | Out-Null

$wrapperPath = Join-Path $wowooaiBin "wowooai.ps1"
$wrapperContent = @'
# wowooai CLI wrapper — delegates to the uv-managed environment.
$ErrorActionPreference = "Stop"

$wowooaiHome = if ($env:wowooai_HOME) { $env:wowooai_HOME } else { Join-Path $HOME ".wowooai" }
$RealBin   = Join-Path $wowooaiHome "venv\Scripts\wowooai.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "wowooai environment not found at $wowooaiHome\venv"
...
$cmdWrapperPath = Join-Path $wowooaiBin "wowooai.cmd"
$cmdWrapperContent = @"
@echo off
REM wowooai CLI wrapper — delegates to the uv-managed environment.
set "wowooai_HOME=%wowooai_HOME%"
if "%wowooai_HOME%"=="" set "wowooai_HOME=%USERPROFILE%\.wowooai"
set "REAL_BIN=%wowooai_HOME%\venv\Scripts\wowooai.exe"
if not exist "%REAL_BIN%" (
    echo Error: wowooai environment not found at %wowooai_HOME%\venv >&2
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use wowooai."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "wowooai installed successfully!" -ForegroundColor Green
Write-Host ""

Write-Host "  Install location:  " -NoNewline; Write-Host "$wowooaiHome" -ForegroundColor White
...
Write-Host ""
Write-Host "  wowooai init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  wowooai app"  -ForegroundColor White -NoNewline; Write-Host "        # start wowooai"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "wowooai uninstall" -ForegroundColor White

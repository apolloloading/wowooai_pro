...
info()  { printf "${GREEN}[wowooai]${RESET} %s\n" "$*"; }
warn()  { printf "${YELLOW}[wowooai]${RESET} %s\n" "$*"; }
error() { printf "${RED}[wowooai]${RESET} %s\n" "$*" >&2; }
die()   { error "$@"; exit 1; }

# ── Defaults ──────────────────────────────────────────────────────────────────
wowooai_HOME="${wowooai_HOME:-$HOME/.wowooai}"
wowooai_VENV="$wowooai_HOME/venv"
wowooai_BIN="$wowooai_HOME/bin"
PYTHON_VERSION="3.12"
wowooai_REPO="https://github.com/agentscope-ai/wowooai.git"
...
printf "${GREEN}[wowooai]${RESET} Installing wowooai into ${BOLD}%s${RESET}\n" "$wowooai_HOME"
...
if [ -d "$wowooai_VENV" ]; then
    info "Existing environment found, upgrading..."
else
    info "Creating Python $PYTHON_VERSION environment..."
fi

uv venv "$wowooai_VENV" --python "$PYTHON_VERSION" --quiet

# Verify the venv was created
[ -x "$wowooai_VENV/bin/python" ] || die "Failed to create virtual environment"
info "Python environment ready ($("$wowooai_VENV/bin/python" --version))"
...
## Ensure console frontend assets are in src/wowooai/console/ for source installs.
...
prepare_console() {
    ...
    local console_dest="$repo_dir/src/wowooai/console"
...
cleanup_console() {
    ...
    if [ "$_CONSOLE_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/wowooai/console/"*
    fi
}

if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        info "Installing wowooai from local source: $SOURCE_DIR"
        prepare_console "$SOURCE_DIR"
        info "Installing package from source..."
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$wowooai_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        cleanup_console "$SOURCE_DIR"
    else
        info "Installing wowooai from source (GitHub)..."
        CLONE_DIR="$(mktemp -d)"
        trap 'rm -rf "$CLONE_DIR"' EXIT
        git clone --depth 1 "$wowooai_REPO" "$CLONE_DIR"
        prepare_console "$CLONE_DIR"
        info "Installing package from source..."
        uv pip install "${CLONE_DIR}${EXTRAS_SUFFIX}" --python "$wowooai_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        # CLONE_DIR is cleaned up by trap; no need for cleanup_console
    fi
else
    PACKAGE="wowooai"
    if [ -n "$VERSION" ]; then
        PACKAGE="wowooai==$VERSION"
    fi

    info "Installing ${PACKAGE}${EXTRAS_SUFFIX} from PyPI..."
    uv pip install "${PACKAGE}${EXTRAS_SUFFIX}" --python "$wowooai_VENV/bin/python" --prerelease=allow --quiet --index-url "$PYPI_MIRROR" --refresh-package wowooai
fi

# Verify the CLI entry point exists
[ -x "$wowooai_VENV/bin/wowooai" ] || die "Installation failed: wowooai CLI not found in venv"
info "wowooai installed successfully"
...
if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    ...
    CONSOLE_CHECK="$("$wowooai_VENV/bin/python" -c "import importlib.resources, wowooai; p=importlib.resources.files('wowooai')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
cat > "$wowooai_BIN/wowooai" << 'WRAPPER'
#!/usr/bin/env bash
# wowooai CLI wrapper — delegates to the uv-managed environment.
set -euo pipefail

wowooai_HOME="${wowooai_HOME:-$HOME/.wowooai}"
REAL_BIN="$wowooai_HOME/venv/bin/wowooai"
...
chmod +x "$wowooai_BIN/wowooai"
info "Wrapper created at $wowooai_BIN/wowooai"

# ── Step 5: Update PATH in shell profile ─────────────────────────────────────
PATH_ENTRY="export PATH=\"\$HOME/.wowooai/bin:\$PATH\""

add_to_profile() {
    local profile="$1"
    if [ -f "$profile" ] && grep -qF '.wowooai/bin' "$profile"; then
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}wowooai installed successfully!${RESET}\n"
echo ""

# Install summary
printf "  Install location:  ${BOLD}%s${RESET}\n" "$wowooai_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$wowooai_VENV/bin/python" --version 2>&1)"
...
echo "Then run:"
echo ""
printf "  ${BOLD}wowooai init${RESET}       # first-time setup\n"
printf "  ${BOLD}wowooai app${RESET}        # start wowooai\n"
echo ""
printf "To upgrade later, re-run this installer.\n"
printf "To uninstall, run: ${BOLD}wowooai uninstall${RESET}\n"

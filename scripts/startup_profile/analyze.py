...
# -*- coding: utf-8 -*-
"""
wowooai Startup Performance Analyzer
...
def parse_import_log(log_file):
    ...
    # Filter wowooai and third-party
    wowooai = [i for i in imports if "wowooai" in i["package"]]
    third_party = [
        i
        for i in imports
    ...
    wowooai.sort(key=lambda x: x["cumulative_ms"], reverse=True)
    third_party.sort(key=lambda x: x["cumulative_ms"], reverse=True)

    total_wowooai = sum(
        i["cumulative_ms"] for i in wowooai if i["package"].count(".") == 1
    )
    total_third_party = sum(i["cumulative_ms"] for i in third_party[:10])

    print(
        f"   ✓ {len(imports)} imports, {len(wowooai)} wowooai, {len(third_party)} third-party",
    )

    return {
        "wowooai_imports": wowooai,
        "third_party_imports": third_party,
        "summary": {
            "total_ms": total_wowooai + total_third_party,
            "total_wowooai_ms": total_wowooai,
...
def parse_execution_trace(trace_file):
    ...
    try:
        ...
        wowooai_funcs = [f for f in functions if "wowooai" in f["function"]]

        print(f"   ✓ {len(functions)} functions, {len(wowooai_funcs)} wowooai")

        return {
            "wowooai_functions": wowooai_funcs[:50],
...
def main():  # pylint: disable=too-many-statements
    """Main function."""
    print("=" * 60)
    print("🐾 wowooai Startup Performance Analyzer")
    print("=" * 60)
    print()

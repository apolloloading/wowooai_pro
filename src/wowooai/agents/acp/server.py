# -*- coding: utf-8 -*-
"""wowooai ACP Agent server.

Exposes wowooai as an ACP-compliant agent that external clients
...
def _msg_to_updates(  # pylint: disable=too-many-branches
    ...
    """Convert a wowooai Msg into ACP session update(s).
...
class wowooaiACPAgent(Agent):
    ...
    async def _ensure_workspace(self) -> Any:
        ...
        logger.info(
            "wowooai ACP Agent workspace started: agent_id=%s workspace=%s",
            agent_id,
            workspace_dir,
        )
        return runner
...
async def run_wowooai_agent(
    agent_id: str | None = None,
    workspace_dir: Path | None = None,
) -> None:
    """Entry point: run wowooai as an ACP agent over stdio."""
    agent = wowooaiACPAgent(
        agent_id=agent_id,

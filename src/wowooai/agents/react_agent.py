# -*- coding: utf-8 -*-
"""wowooai Agent - Main agent implementation.

This module provides the main wowooaiAgent class built on ReActAgent,
...
class wowooaiAgent(ToolGuardMixin, ReActAgent):
    """wowooai Agent with integrated tools, skills, and memory management.
    ...
    Python's MRO: wowooaiAgent → ToolGuardMixin → ReActAgent.  If you
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def _rebuild_mcp_client(client: Any) -> Any | None:
        ...
        rebuild_info = getattr(client, "_wowooai_rebuild_info", None)
        ...
        try:
            if transport == "stdio":
                ...
                setattr(rebuilt_client, "_wowooai_rebuild_info", rebuild_info)
                return rebuilt_client
    ...
    def _fix_stringified_json_args(tool_call) -> None:
        ...
        if not isinstance(inp, dict):
            return
        for key in wowooaiAgent._PLAN_JSON_KEYS:
            val = inp.get(key)
    ...
    def _set_formatter_media_strip(self, enabled: bool) -> None:
        ...
        if formatter is None:
            return
        setattr(formatter, "_wowooai_force_strip_media", enabled)
    ...
    def _strip_tool_use_from_msg(msg: Msg) -> Msg:
        ...
        if isinstance(msg.content, str):
            msg.content += wowooaiAgent._ROUND_END_NOTICE
            return msg
        ...
        filtered.append(
            {"type": "text", "text": wowooaiAgent._ROUND_END_NOTICE},
        )
        msg.content = filtered
        return msg
    ...
    async def reply(
        ...
        logger.info("wowooaiAgent.reply: max_iters=%s", self.max_iters)

...
description: Use this skill to proactively send a one-way message to a user/session/channel, usually only when the user explicitly asks to send to a channel/session or when proactive notification is needed. First query sessions with wowooai chats list, then push with wowooai channels send.
metadata:
...
### Should Not Use
- If you are simply replying in the current session, **do not use `wowooai channels send`**
...
```bash
wowooai chats list --agent-id <your_agent> --channel <channel>
```

You can also filter by user:

```bash
wowooai chats list --agent-id <your_agent> --user-id <user_id>
```

### 2) Send a message

```bash
wowooai channels send \
  --agent-id <your_agent> \
  --channel <channel> \
  --target-user <user_id> \
...
`wowooai channels send` requires all of the following:
- `--agent-id`
- `--channel`
- `--target-user`
- `--target-session`
...
`wowooai channels send` only sends — it does not wait for a reply.
...
```bash
wowooai chats list --agent-id notify_bot --channel feishu
...
```bash
wowooai chats list --agent-id task_bot --channel console
...
```bash
wowooai chats list --agent-id analyst_bot --user-id alice
...
If you are replying to the user in the current session, do not use `wowooai channels send`.
...
```bash
wowooai chats list --agent-id <your_agent>
```
...
```bash
wowooai channels list --agent-id <your_agent>
```
...
- **wowooai agents chat**: sends to another agent, two-way, returns a reply
- **wowooai channels send**: sends to a user/session/channel, one-way, no reply

**Selection principle**:
- Need to collaborate with another agent → `wowooai agents chat`
- Need to proactively push a message to a user → `wowooai channels send`
...
- `--target-user`: target user ID (obtained from `wowooai chats list`)
- `--target-session`: target session ID (obtained from `wowooai chats list`)
...
```bash
wowooai channels -h
wowooai channels send -h
wowooai chats -h
wowooai chats list -h
```

---
name: cron
description: Use this skill only for scheduled or recurring tasks. Manage jobs with wowooai cron list/create/get/state/pause/resume/delete/run, and always pass --agent-id explicitly.
metadata:
...
All `wowooai cron` commands **must** include:
...
5. Create the task with wowooai cron create
...
- Before modifying/pausing/deleting, run `wowooai cron list --agent-id <agent_id>` first
- To troubleshoot issues, use `wowooai cron state <job_id> --agent-id <agent_id>`

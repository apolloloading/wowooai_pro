---
name: guidance
description: "Answer user questions about wowooai installation and configuration: first locate and read local documentation, then distill the answer; if local information is insufficient, fall back to the official website documentation."
metadata:
...
# wowooai Installation and Configuration Q&A Guide

Use this skill when the user asks about **wowooai installation, initialization, environment configuration, dependency requirements, or common configuration options**.
...
DOC_DIR=$(find ~/.wowooai/memory/ -type d -name "docs")
```
...
Run the following script logic to obtain the variable $wowooai_ROOT:

```bash
# Get the absolute path of the binary
COP_PATH=$(which wowooai 2>/dev/null || whereis wowooai | awk '{print $2}')

# Logical deduction: if the path contains .wowooai/bin/wowooai, the root is three levels up
# Example: /path/to/wowooai/.wowooai/bin/wowooai -> /path/to/wowooai
if [[ "$COP_PATH" == *".wowooai/bin/wowooai" ]]; then
    wowooai_ROOT=$(echo "$COP_PATH" | sed 's/\/\.wowooai\/bin\/wowooai//')
else
    # Fallback: try to get the parent of the parent directory
    wowooai_ROOT=$(dirname $(dirname "$COP_PATH") 2>/dev/null || echo ".")
fi

echo "Detected wowooai Root: $wowooai_ROOT"
```

Verify and list the documentation directory:
Use the derived $wowooai_ROOT to locate the documentation:

```bash
# Construct the standard documentation path
DOC_DIR="$wowooai_ROOT/website/public/docs/"
...
if [ -d "$DOC_DIR" ]; then
    ...
    find "$wowooai_ROOT" -type d -name "docs" | grep "website/public/docs"
fi
```

**If project documentation does not exist, search the working directory**

If documentation is still not found, search for available documentation content under the wowooai installation path:

...
description: Use this skill when another agent's expertise or context is needed, or when the user explicitly asks to involve another agent. First list agents, then use wowooai agents chat for two-way communication with replies.
metadata:
...
```bash
wowooai agents chat --background \
  --from-agent <your_agent> \
  --to-agent <target_agent> \
  --text "[Agent <your_agent> requesting] ..."
```
...
```bash
wowooai agents chat --background --task-id <task_id>
```
...
|-----------|-------------|---------|
| Simple quick query | Real-time mode | `wowooai agents chat` |
| Complex task (data analysis, batch processing, etc.) | Background mode | `wowooai agents chat --background` |
...
2. wowooai agents list
3. wowooai agents chat to start a conversation
...
2. wowooai agents list
3. wowooai agents chat --background to submit the task
...
`wowooai agents chat` must include all of the following:
- `--from-agent`
- `--to-agent`
- `--text`
...
Check status with:
  wowooai agents chat --background --task-id 20802ea3-...
```
...
sleep 30 && wowooai agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
wowooai agents chat --background --task-id <id>
```

**Method 2: Timed polling** (if you must wait)
```bash
# Incrementally increasing intervals, fast first then slow
sleep 10 && wowooai agents chat --background --task-id <id>
sleep 20 && wowooai agents chat --background --task-id <id>
sleep 30 && wowooai agents chat --background --task-id <id>
```
...
```bash
wowooai agents -h
wowooai agents list -h
wowooai agents chat -h
```

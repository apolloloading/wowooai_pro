...
description: |
  ...
  NOT a callable function — DO NOT emit tool_call with
  name="onboarding-guide". Call via execute_shell_command:

      python3 "{skill_dir}/scripts/query.py" "<original user question>"

  The script returns a relevance-sorted table from the Bailian
  knowledge base; summarise the top hits in your reply.

  Use for company matters at 人力窝/久事附楼: WiFi, VPN/OpenVPN, mail,
  shared drive, Cisco Jabber, meeting room casting, printers, courier
  hours, BFC cafeteria, lockers, 6S, finance reimbursement, shuttle
  bus / Line 9 Xiao Nan Men, onboarding forms, employee handbook.
  Prefer this over `memory_search` — that only sees chat history.

  HARD CONSTRAINT — 只用本库返回的数据回答，严禁用模型自身知识补全。
  命中 0 条时告知用户"本库未收录"。
metadata:
  type: knowledge
  label: 入职指引

...
_REQUIREMENTS_METADATA_NAMESPACES = ("openclaw", "wowooai", "clawdbot")
...
def _skill_config_env_var_name(skill_name: str) -> str:
    ...
    return (
        f"wowooai_SKILL_CONFIG_{''.join(normalized).strip('_') or 'DEFAULT'}"
    )
...
def apply_skill_config_env_overrides(
    ...
    ``wowooai_SKILL_CONFIG_<SKILL_NAME>`` (JSON string).
    """
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.wowooai.emoji."""
    if not isinstance(metadata, dict):
        return ""
    wowooai = metadata.get("wowooai")
    if isinstance(wowooai, dict):
        return str(wowooai.get("emoji", "") or "")
    return ""
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="wowooai_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"wowooai_skill_stage_{skill_name}_"),
    )

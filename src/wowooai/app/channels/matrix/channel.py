# -*- coding: utf-8 -*-
"""
MatrixChannel: wowooai BaseChannel implementation for Matrix (via matrix-nio).
...
logger = logging.getLogger("wowooai.channels.matrix")
...
# Known wowooai slash commands — used to decide whether to strip
# @mention prefix
...
class MatrixChannelConfig:
    ...
    def __init__(self, raw: dict[str, Any]) -> None:
        ...
        self.device_name: str = raw.get("device_name", "wowooai-worker")
...
class MatrixChannel(BaseChannel):
    """wowooai channel that connects to a Matrix homeserver via matrix-nio."""
    ...
    async def _on_room_event(
        ...
        # Skip history prepend for slash commands — wowooai's command parser
    ...
    # native content_parts → wowooai Content; same
    # vision_enabled guard as inbound media for image parts (§11).
    # ------------------------------------------------------------------

    # pylint: disable=too-many-return-statements
    def _build_content_part(self, p: dict[str, Any]) -> Any:
        """Convert a native content-part dict to a wowooai Content object."""
        t = p.get("type")
    ...
    def build_agent_request_from_native(self, native_payload: Any) -> Any:
        ...
        # in the same room share one session (wowooai keys session state on

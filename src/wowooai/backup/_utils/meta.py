...
def get_wowooai_version() -> str:
    """Return the installed wowooai package version, or ``'unknown'``."""
    try:
...
def generate_backup_id() -> str:
    ...
    Format: ``wowooai-{version}-{YYYYMMDDTHHmmssZ}-{short8}``

    Example: ``wowooai-1.2.3-20260420T093000Z-ab12cd34``
    """
    ver = re.sub(r"[^a-zA-Z0-9._-]", "_", get_wowooai_version())
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    short = uuid4().hex[:8]
    return f"wowooai-{ver}-{ts}-{short}"
...
def finalize_backup_meta(meta, agent_count: int) -> None:
    ...
    meta.wowooai_version = get_wowooai_version()

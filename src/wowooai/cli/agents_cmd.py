...
def _submit_background_task(
    ...
    try:
        ...
        click.echo(f"  wowooai agents chat --background --task-id {task_id}")
...
def _check_task_status(
    ...
    try:
        ...
        if status == "finished":
            ...
            click.echo(
                f"  wowooai agents chat --background --task-id {task_id}",
            )
...
def agents_group() -> None:
    ...
    \b
    Examples:
      wowooai agents chat --from-agent bot_a --to-agent bot_b --text "..."
      wowooai agents create --name "Research Bot" --agent-id research_bot
      wowooai agents delete research_bot
      wowooai agents list
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      wowooai agents list
      wowooai agents list --base-url http://192.168.1.100:8088
...
def delete_cmd(
    ...
    \b
    AGENT_ID  Configured agent ID, obtainable via `wowooai agents list`.

    \b
    Examples:
      wowooai agents delete research
      wowooai agents delete research --remove-workspace
      wowooai agents delete research --yes
    """
...
def chat_cmd(
    ...
    \b
    Prerequisites:
      1. Use 'wowooai agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'wowooai chats list' to find existing sessions (optional)

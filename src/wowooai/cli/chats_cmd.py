...
def chats_group() -> None:
    ...
    \b
    Common examples:
      wowooai chats list                    # List all chats
      wowooai chats list --user-id alice    # Filter by user
      wowooai chats get <chat_id>           # View details
      wowooai chats create --session-id s1 --user-id u1
      wowooai chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      wowooai chats list
      wowooai chats list --user-id alice
      wowooai chats list --channel discord
      wowooai chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `wowooai chats list`.

    \b
    Examples:
      wowooai chats get 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
...
def create_chat(
    ...
    Inline creation examples:
      wowooai chats create --session-id "discord:alice" \\
    ...
    JSON file creation example:
      wowooai chats create -f chat.json
    """
    base_url = resolve_base_url(ctx, base_url)
...
def update_chat(
    ...
    \b
    Examples:
      wowooai chats update <chat_id> --name "Renamed Chat"
    """
    base_url = resolve_base_url(ctx, base_url)
    headers = {"X-Agent-Id": agent_id}
    payload = {"name": name}
...
def delete_chat(
    ...
    \b
    Examples:
      wowooai chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}

# -*- coding: utf-8 -*-
"""`wowooai doctor` — read-only checks.

`wowooai doctor fix` — conservative repairs with backup.
"""
...
def _doctor_server_python_mismatch_note(
    ...
    if server_exe and doctor_exe:
        if not _same_python_executable(doctor_exe, server_exe):
            return (
                "This `wowooai doctor` is not using the same Python "
                "executable as the running `wowooai app` — diagnostics and "
    ...
    if doctor_env.strip() != server_env.strip():
        return (
            ...
            f"`wowooai app` (doctor: {doctor_env!r}; server: "
            f"{server_env!r}). "
...
def _check_web_auth(base: str) -> tuple[bool, str]:
    ...
    if not has_registered_users():
        return (
            ...
            f"        1) Start `wowooai app`, open {base}/ in a browser.\n"
            "        2) Complete registration (single user) on the login "
            "page.\n"
            "        For automation, set wowooai_AUTH_USERNAME and "
            "wowooai_AUTH_PASSWORD (legacy wowooai_* names still work) — the "
...
async def _check_active_llm(
    ...
    if (
        ...
        return (
            False,
            "no active LLM slot — run `wowooai models list` and configure "
            "an active model",
            [],
        )
    ...
    if deep and pid in ("wowooai-local", "wowooai-local"):
        deep_notes = wowooai_local_llm_deep_notes()
...
def run_doctor_checks(
    ...
    """Run read-only ``wowooai doctor`` checks (no disk mutations)."""
    ...
    if config_ok:
        ...
        _doctor_fix_hint(
            ...
            "`wowooai doctor fix --dry-run --help` and `--only`.",
        )
    ...
    if raw_cfg is not None:
        ...
        if unknown:
            ...
            _doctor_fix_hint(
                ...
                "(`wowooai doctor` and `doctor fix` do not strip unknown keys "
                "yet).",
            )
    ...
    if config_ok:
        ...
        if ws_ok:
            ...
            _doctor_fix_hint(
                "Preview the plan (no writes): `wowooai doctor fix --dry-run "
        ...
        if not ext_nonempty:
            click.echo(
                click.style("OK", fg="green")
                + " — no extension notes (register via wowooai.doctor entry "
                "points or register_doctor_contribution; legacy "
                "wowooai.doctor is still loaded)",
            )
        else:
        ...
        click.echo(
            ...
            + _skipped_when_cfg_invalid
            + ". Fix the config file, then re-run `wowooai doctor`.",
        )
        click.echo("\n=== Browser (browser_use / Playwright) ===")
    ...
    if wd_ok:
        ...
        _doctor_fix_hint(
            "Fix: set `wowooai_WORKING_DIR` (or legacy `wowooai_WORKING_DIR`) "
            "or run `wowooai init`. "
            "Preview the plan (no writes): `wowooai doctor fix --dry-run "
    ...
    if llm_ok:
        ...
        _doctor_fix_hint(
            "`wowooai models list` / console model settings — not a "
            "filesystem fix.",
        )
    ...
    try:
        ...
        click.echo(
            f"Hint: start the server with `wowooai app` (default {base}).",
            err=True,
        )
    else:
...
@click.option(
    ...
    help=(
        ...
        "uses --timeout) and, when the active model is wowooai-local, "

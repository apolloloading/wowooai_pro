# -*- coding: utf-8 -*-
"""Extensible hooks for `wowooai doctor`
(entry points + programmatic registration).

Plugins can expose a setuptools entry point in group ``wowooai.doctor``::

    [project.entry-points."wowooai.doctor"]
    my_pkg = "my_pkg.doctor:doctor_notes"

The group ``wowooai.doctor`` is still discovered when present (legacy plugins).
...
def _load_entry_point_functions() -> list[tuple[str, DoctorNotesFn]]:
    ...
    seen: set[str] = set()
    for group in ("wowooai.doctor", "wowooai.doctor"):

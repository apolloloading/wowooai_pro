...
def _check_wowooai_not_running():
    """Check if wowooai is not running, exit if it is."""
    from ..config.utils import is_wowooai_running

    if is_wowooai_running():
        click.echo(
            "❌ wowooai is currently running. Please stop it first:",
            err=True,
        )
        click.echo("   wowooai shutdown", err=True)
        click.echo(
            "\n💡 Plugin operations are only allowed when wowooai is stopped.",
        )
        raise click.Abort()
...
def install(source: str, force: bool):
    ...
    Examples:
        wowooai plugin install examples/plugins/idealab-provider
        wowooai plugin install /path/to/plugin
        wowooai plugin install https://example.com/plugin.zip
    """
    from ..config.utils import get_plugins_dir

    # Check if wowooai is running
    _check_wowooai_not_running()
    ...
    click.echo("\n💡 Next steps:")
    click.echo("   1. Restart wowooai to load the plugin")
...
def uninstall(plugin_id: str):
    ...
    try:
        ...
        click.echo("💡 Restart wowooai to apply changes")
    except Exception as e:

...
def _matches_wowooai_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a wowooai CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m wowooai {subcommand}",
            f" wowooai {subcommand}",
            f"__main__.py {subcommand}",
            f'wowooai.exe" {subcommand}',
            f"wowooai.exe {subcommand}",
        )
    )


def _is_wowooai_service_command(command: str) -> bool:
    """Return whether the command line looks like a local wowooai app."""
    return _matches_wowooai_cli_command(command, "app")


def _is_wowooai_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a wowooai CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "wowooai" or _matches_wowooai_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local wowooai service."""
    if not host:
        return []

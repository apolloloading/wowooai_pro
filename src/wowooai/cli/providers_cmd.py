    ...
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'wowooai[local]'",
                fg="red",
            ),
        )
...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'wowooai models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'wowooai models config' to set one.",
        )
        return
    try:
    ...
    for defn in _all_provider_objects(manager):
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'wowooai models download' to add models.")
        else:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'wowooai models add-model' to add models, "
        "then 'wowooai models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      wowooai models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
      wowooai models download Qwen/Qwen2-0.5B-Instruct-GGUF --source modelscope
    """
    ...
    click.echo(
        ...
        "  wowooai models set-llm  (select 'wowooai-local' provider)",
    )
...
def list_local_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'wowooai models download <repo_id>' to download one.")
        return

# -*- coding: utf-8 -*-
"""wowooai uninstall — remove the wowooai environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove wowooai PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# wowooai" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# wowooai\nexport PATH=\"\$HOME/\.wowooai/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove wowooai environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL wowooai data in {wd}")
    else:
        click.echo(
            "This will remove the wowooai Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("wowooai uninstalled. Please restart your terminal.")

...
from .process_utils import (
    ...
    _is_wowooai_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/wowooai/json"
...
class InstallInfo:
    """Information about the current wowooai installation."""
...
class RunningServiceInfo:
    """Detected wowooai service endpoint state."""
...
def _fetch_latest_version() -> str:
    """Fetch the latest published wowooai version from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest wowooai version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest wowooai version: {exc}",
        ) from exc
    version = str(data.get("info", {}).get("version", "")).strip()
    if not version:
        raise click.ClickException(
            "Unable to determine the latest wowooai version.",
        )
    return version
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("wowooai")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running wowooai HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local wowooai service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_wowooai_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a wowooai HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running wowooai service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"wowooai service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `wowooai shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING wowooai SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `wowooai shutdown` will forcibly terminate the current "
        "wowooai backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `wowooai shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `wowooai shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "wowooai"]
    ...
    click.echo("")
    click.echo("Running `wowooai shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `wowooai shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`wowooai shutdown` failed. Please stop the running wowooai "
            "service manually before running `wowooai update`.",
        )
...
def _build_upgrade_command(
    ...
    package_spec = f"wowooai=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from wowooai.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[wowooai] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[wowooai] Updating wowooai "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[wowooai] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[wowooai] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[wowooai] Update completed successfully.")
        click.echo(
            "[wowooai] Please restart any running wowooai service "
            "to use the new version.",
        )
    else:
        click.echo(f"[wowooai] Update failed with exit code {return_code}.")
        click.echo(
            "[wowooai] Please fix the error above and run "
            "`wowooai update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool) -> None:
    """Upgrade wowooai in the current Python environment."""
    ...
    if version_check is False:
        click.echo("wowooai is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `wowooai update`, or rerun "
                "without `--yes` to confirm a forced `wowooai shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `wowooai shutdown`. "
                "Please stop it manually before running `wowooai update`.",
            )

    if not yes and not click.confirm(
        f"Update wowooai to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return

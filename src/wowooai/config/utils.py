...
def _normalize_working_dir_bound_paths(data: object) -> object:
    ...
    This keeps wowooai_WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.wowooai/media" or
    "/Users/x/.wowooai/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_root_tilde = "~/.wowooai"
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    wowooai.channels), filtered by wowooai_ENABLED_CHANNELS or
    wowooai_DISABLED_CHANNELS when set.

    * wowooai_ENABLED_CHANNELS — whitelist (only these channels are active).
    * wowooai_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, wowooai_ENABLED_CHANNELS takes precedence.
    * If neither is set, all discovered channels are returned.
    ...
    raw_enabled = EnvVarLoader.get_str("wowooai_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
...
def is_running_in_container() -> bool:
    ...
    Prefer env wowooai_RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def is_wowooai_running() -> bool:
    """Check if wowooai is currently running by checking API availability.

    Returns:
        True if wowooai is running, False otherwise
    """
    try:

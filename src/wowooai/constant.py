...
def _get_env(key: str, default: str = "") -> str:
    """Look up an env var with automatic wowooai_ legacy fallback.

    Primary key is always used as-is.  When the primary key starts with
    ``wowooai_``, the corresponding ``wowooai_`` variant is transparently
    checked as a fallback so that existing deployments keep working.
    """
    if key in os.environ:
        return os.environ[key]
    if key.startswith("wowooai_"):
        legacy_key = "wowooai_" + key[len("wowooai_") :]
...
class EnvVarLoader:
    ...
    and defaults.  Pass wowooai_* keys; wowooai_* legacy variants are
...
# 1. wowooai_WORKING_DIR / wowooai_WORKING_DIR env var is set → use it
# 2. ~/.wowooai exists (legacy installation) → use it as-is
# 3. Default → ~/.wowooai
_explicit_working_dir = _get_env("wowooai_WORKING_DIR")
if _explicit_working_dir:
    WORKING_DIR = Path(_explicit_working_dir).expanduser().resolve()
else:
    _legacy_wowooai_dir = Path("~/.wowooai").expanduser()
    if _legacy_wowooai_dir.exists():
        WORKING_DIR = _legacy_wowooai_dir.resolve()
    else:
        WORKING_DIR = Path("~/.wowooai").expanduser().resolve()
SECRET_DIR = (
    Path(
...
JOBS_FILE = EnvVarLoader.get_str("wowooai_JOBS_FILE", "jobs.json")

CHATS_FILE = EnvVarLoader.get_str("wowooai_CHATS_FILE", "chats.json")


# Builtin Q&A helper profile.  agent_id keeps "wowooai" prefix for existing
...
LEGACY_QA_AGENT_ID = "wowooai_QA_Agent_0.1beta1"

TOKEN_USAGE_FILE = EnvVarLoader.get_str(
    "wowooai_TOKEN_USAGE_FILE",
    "token_usage.json",
)

CONFIG_FILE = EnvVarLoader.get_str("wowooai_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = EnvVarLoader.get_str("wowooai_HEARTBEAT_FILE", "HEARTBEAT.md")
...
DOCS_ENABLED = EnvVarLoader.get_bool("wowooai_OPENAPI_DOCS", False)
...
# Custom channel modules (installed via `wowooai channels install`); manager
# loads BaseChannel subclasses from here.
CUSTOM_CHANNELS_DIR = WORKING_DIR / "custom_channels"

# Plugin directory (installed via `wowooai plugin install`)
...
MEMORY_COMPACT_KEEP_RECENT = EnvVarLoader.get_int(
    "wowooai_MEMORY_COMPACT_KEEP_RECENT",
    3,
    min_value=0,
)
...
# Example: wowooai_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = EnvVarLoader.get_str("wowooai_CORS_ORIGINS", "").strip()

# LLM API retry configuration
LLM_MAX_RETRIES = EnvVarLoader.get_int(
    "wowooai_LLM_MAX_RETRIES",
    3,
    min_value=0,
)

LLM_BACKOFF_BASE = EnvVarLoader.get_float(
    "wowooai_LLM_BACKOFF_BASE",
    1.0,
    min_value=0.1,
)

LLM_BACKOFF_CAP = EnvVarLoader.get_float(
    "wowooai_LLM_BACKOFF_CAP",
    10.0,
    min_value=0.5,
)
...
try:
    TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS = max(
        float(
            _get_env("wowooai_TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS", "300"),
        ),
        1.0,
    )
except (TypeError, ValueError):
...
try:
    TOOL_GUARD_APPROVAL_HEARTBEAT_INTERVAL = max(
        float(
            _get_env("wowooai_TOOL_GUARD_APPROVAL_HEARTBEAT_INTERVAL", "15"),
        ),
        5.0,
    )
except (TypeError, ValueError):

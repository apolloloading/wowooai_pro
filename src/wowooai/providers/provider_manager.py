...
PROVIDER_wowooai = OpenAIProvider(
    id="wowooai-local",
    name="wowooai Local",
    is_local=True,
...
class ProviderManager:  # pylint: disable=too-many-public-methods
    ...
    def _init_builtins(self):
        self._add_builtin(PROVIDER_wowooai)
    ...
    def _normalize_provider_id(provider_id: str) -> str:
        ...
        Maps legacy 'wowooai-local' to 'wowooai-local'.
        """
        if provider_id == "wowooai-local":
            return "wowooai-local"
        return provider_id
    ...
    def start_local_model_resume(self, local_manager) -> None:
        ...
        task = asyncio.create_task(
            ...
            name="wowooai-local-model-resume",
        )
    ...
    def _migrate_wowooai_config(self) -> None:
        """Migrate wowooai-local provider config to wowooai-local."""
        # 1. Migrate active model configuration (only provider_id)
        if (
            self.active_model
            and self.active_model.provider_id == "wowooai-local"
        ):
            self.active_model.provider_id = "wowooai-local"
            self.save_active_model(self.active_model)
            logger.info(
                "Migrated active model provider from "
                "'wowooai-local' to 'wowooai-local'",
            )

        # 2. Migrate stored provider config file
        wowooai_config_path = self.builtin_path / "wowooai-local.json"
        if not wowooai_config_path.exists():
            return

        try:
            # Load old config and apply to new provider instance
            with open(wowooai_config_path, "r", encoding="utf-8") as f:
                old_config = json.load(f)

            # Get the new built-in provider instance
            provider = self.builtin_providers.get("wowooai-local")
            if not provider:
                return
            ...
            wowooai_config_path.unlink()
            logger.info(
                "Migrated provider config from "
                "'wowooai-local.json' to 'wowooai-local.json'",
            )
        except Exception as exc:
            logger.warning("Failed to migrate wowooai-local config: %s", exc)
    ...
    def _migrate_legacy_providers(self):
        ...
        if legacy_path.exists() and legacy_path.is_file():
            ...
            if active_model:
                try:
                    # Convert legacy wowooai-local provider_id
                    if active_model.get("provider_id") == "wowooai-local":
                        active_model["provider_id"] = "wowooai-local"
    ...
    def _init_from_storage(self):
        ...
        # Migrate wowooai-local to wowooai-local for backwards compatibility
        self._migrate_wowooai_config()
    ...
    async def _resume_local_model(self, local_manager) -> None:
        ...
        local_models = self.get_provider("wowooai-local").extra_models

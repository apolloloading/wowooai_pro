...
"""
Global pytest fixtures for wowooai test suite.

This module provides shared fixtures for testing wowooai components.
...
def temp_workspace() -> Generator[Path, None, None]:
    ...
    """
    temp_dir = tempfile.mkdtemp(prefix="wowooai_test_")
    try:
        yield Path(temp_dir)
    finally:
...
@pytest.fixture
def temp_wowooai_home(
    monkeypatch: pytest.MonkeyPatch,
) -> Generator[Path, None, None]:
    """Provide an isolated wowooai HOME environment.

    Creates a temporary directory and sets it as both HOME and wowooai_HOME
    environment variables. Also clears any sensitive environment variables
    that could interfere with tests.

    Yields:
        Path to the temporary wowooai home directory.

    Example:
        def test_config_loading(temp_wowooai_home):
            # This test runs in an isolated environment
            config_path = temp_wowooai_home / ".wowooai" / "config.yaml"
            # ... test config operations
    """
    temp_dir = tempfile.mkdtemp(prefix="wowooai_home_")
    temp_path = Path(temp_dir)

    # Create standard subdirectories
    (temp_path / ".wowooai").mkdir(exist_ok=True)
    (temp_path / ".wowooai" / "skills").mkdir(exist_ok=True)
    (temp_path / ".wowooai" / "logs").mkdir(exist_ok=True)

    # Store original values (for potential future use)
    _ = os.environ.get("HOME")  # noqa: F841
    _ = os.environ.get("wowooai_HOME")  # noqa: F841

    # Set isolated environment
    monkeypatch.setenv("HOME", temp_dir)
    monkeypatch.setenv("wowooai_HOME", str(temp_path / ".wowooai"))
...
def minimal_config() -> dict[str, Any]:
    """Provide a minimal valid wowooai configuration.
...
def clean_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    Example:
        def test_env_loading(clean_env, monkeypatch):
            monkeypatch.setenv("wowooai_CONFIG", "/tmp/config.yaml")
            # Test config loading from env
    """
    # Clear wowooai-specific environment variables
    vars_to_clear = [
        "wowooai_HOME",
        "wowooai_CONFIG",
        "wowooai_LOG_LEVEL",

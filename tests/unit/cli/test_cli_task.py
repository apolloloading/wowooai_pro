# -*- coding: utf-8 -*-
"""Tests for the ``wowooai task`` headless CLI command."""
...
from wowooai.cli.task_cmd import _read_instruction
...
def test_task_rejects_empty_instruction(monkeypatch) -> None:
    monkeypatch.setattr(
        "wowooai.config.config.load_agent_config",
        MagicMock(),
    )
...
def test_model_flag_overrides_agent_config(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "wowooai.cli.task_cmd._run_task",
        AsyncMock(
...
def test_output_dir_writes_result_json(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("wowooai.cli.task_cmd._run_task", _fake_run_task)
...
def _build_guarded_agent(request_context: dict):
    from wowooai.agents.tool_guard_mixin import ToolGuardMixin
...
async def test_tool_guard_not_bypassed_without_flag():
    ...
    with patch(
        "wowooai.security.tool_guard.engine.get_guard_engine",
    ) as mock_engine_fn:
        ...
        with patch("wowooai.app.approvals.get_approval_service"):
...
def test_e2e_cli_no_guard_and_skills_dir(monkeypatch, tmp_path):
    ...
    async def _spy_run_task(**kwargs):
        ...
        captured["env_tool_guard"] = os.environ.get(
            "wowooai_TOOL_GUARD_ENABLED",
        )
        captured["env_skills_dir"] = os.environ.get("wowooai_SKILLS_DIR")
    ...
    monkeypatch.setattr("wowooai.cli.task_cmd._run_task", _spy_run_task)
...
def test_isolated_workspace_creates_overlay(tmp_path):
    ...
    from wowooai.cli.task_cmd import _isolated_skills_workspace
    from wowooai.agents.skills_manager import resolve_effective_skills
